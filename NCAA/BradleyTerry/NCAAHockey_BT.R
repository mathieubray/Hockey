...
ncaa <- read_csv("NCAA/Data/Schedules/2017-2018_Clean.csv") %>%
  select(Home,at,Away,HomeScore,AwayScore,Date) %>%
  mutate(Date = ymd(Date),
         Result = case_when(HomeScore > AwayScore ~ 2, 
...
ncaa.previous <- read_csv("NCAA/Data/Schedules/2016-2017_Clean.csv") %>%
  select(Home,at,Away,HomeScore,AwayScore,Date) %>%
  mutate(Date = ymd(Date),
         Result = case_when(HomeScore > AwayScore ~ 2, 
                            HomeScore == AwayScore ~ 1,
