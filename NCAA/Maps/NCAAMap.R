...
logo.images <- read_csv("Logos/LogoInfo.csv") %>%
  filter(League == "NCAA") %>%
  mutate(Team = if_else(Team == "Lake Superior","Lake Superior State",Team),
...
players <- read_csv("NCAA/Maps/PlayersHometown_2017-2018.csv") %>%
  ...
  mutate(Label = str_replace_all(Label,pattern="'",replacement=""))
